document.addEventListener('DOMContentLoaded', function(){
    ...
    try {
        ...
        let features = ['auth','database','messenging', 'storage'].filter(
...
document.querySelector(' .addContact').addEventListener('click', event => {
    ...
    if( document.querySelector('#name').value != '' &&
        ...
        document.querySelector('status').innerHTML = 'Some field are missing';
         }
        });
    }catch (e) {
        console.error(e);
...
function contactRecordFromSnapshot(snapshot) {
    ...
    record += '<button><a href onclick="Remove_Data( \''+ key + '\')" /> Remove</a></button>';
    record += '</ul></li>';
    return record;
}
